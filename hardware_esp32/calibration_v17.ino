/*
 * ============================================================
 *  LINE FOLLOW v17 — Junction Cooldown Fix
 *  FIX: Added JUNCTION_COOLDOWN_MS to prevent same junction
 *       being counted multiple times (fixes Mode B skipping)
 * ============================================================
 *  COMMANDS:
...
// ── v17 FIX: cooldown prevents re-counting the same junction ──
unsigned long lastJunctionTime   = 0;
#define       JUNCTION_COOLDOWN_MS 800   // tune: wider junctions → increase
...
void stopAll() {
  ...
  junctionsSeen    = 0;
  junctionFrames   = 0;
  lastJunctionTime = 0;   // v17: reset cooldown timer
}
...
void doLineFollow() {
  ...
  // ── Junction detection (with v17 cooldown) ─────────────────
  if (state == LINE_FOLLOWING) {
    if (sum >= 3) {

      // v17 FIX: ignore if we just confirmed a junction recently
      ...
      if (junctionFrames >= JUNCTION_FRAMES_NEEDED) {
        junctionsSeen++;
        junctionFrames   = 0;
        lastJunctionTime = millis();   // v17: stamp cooldown
      ...
      // Drive straight while in/past junction
      setMotorLeft(baseSpeed);
      setMotorRight(constrain(baseSpeed+rightOffset,0,255));
      return;
    } else {
...
void processCommand(char cmd) {
  switch(cmd) {
    ...
    case 'g':
      ...
      junctionsSeen    = 0;
      junctionFrames   = 0;
      lastJunctionTime = 0;   // v17: reset on each new run
...
void setup() {
  ...
  Serial.println("==========================================");
  Serial.println("  LINE FOLLOW v17 — Junction Cooldown Fix");
  Serial.println("==========================================");
  Serial.println("  A=Room1  B=Room2  C=Room3  then 'g'");
  Serial.println("  n=turn later   m=turn earlier");
  Serial.println("  p=settings     i=IR snap    w=watch IR");
  Serial.println("==========================================");
  printSettings();
}
