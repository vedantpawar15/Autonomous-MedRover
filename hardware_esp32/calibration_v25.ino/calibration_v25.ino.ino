/*
 * ============================================================
 *  LINE FOLLOW v23 — Fix fake room + fix return junction
 *
 *  ROOT CAUSES FIXED:
 *
 *  BUG 1 — Fake "ROOM REACHED" mid-branch (halfway):
 *    Cause: Floor tile grout / gap makes all 5 sensors dark
 *           briefly. Robot was stopped while confirming so it
 *           stayed dark for 600ms → falsely thought = wall.
 *    Fix A: Added BRANCH_MIN_TRAVEL_MS (2000ms) — robot must
 *           spend at least this long driving on the branch
 *           before room-end detection is even armed.
 *    Fix B: LINE_END_CONFIRM_MS increased 600 → 1500ms.
 *           A real wall stays dark permanently. A grout line
 *           will reappear within ~300ms when robot rocks/settles
 *    Fix C: During confirm wait, robot does NOT stay fully
 *           stopped — it holds very slow creep (30 PWM) so
 *           a grout line will be crossed and reappear. A wall
 *           will stay dark regardless.
 *
 *  BUG 2 — Doesn't stop at correct junction on return:
 *    Cause: Wide black tape on vertical line lights 3+ sensors
 *           continuously → checkJunction() fires mid-line
 *           (not at a real T-junction). Robot stops too early.
 *    Fix A: RETURN_JUNCTION_FRAMES increased to 6 (vs 3 normal)
 *           A real T-junction lights sensors for many frames.
 *           Normal wide tape lights ~3 sensors but flickers.
 *    Fix B: RETURN_JUNCTION_COOLDOWN_MS = 2000ms (vs 900ms)
 *           Extra long cooldown prevents the same wide-tape
 *           patch being counted twice.
 *    Fix C: On return, minimum forward travel of
 *           RETURN_MIN_TRAVEL_MS (1500ms) before junction
 *           counting starts — avoids counting the rejoin
 *           junction itself.
 *
 *  OTHER CHANGES vs v22:
 *   - Junction pause kept at 1000ms
 *   - Room wait kept at 7000ms
 *   - U-turn adaptive left→right kept from v22
 *   - New serial prints to help diagnose if issues persist
 *
 *  TRACK LAYOUT:
 ...
 *   4. Wait BRANCH_MIN_TRAVEL_MS before arming end detection
 ...
 *  10. Wait RETURN_MIN_TRAVEL_MS before counting junctions
 *  11. Count N junctions (=targetJunction) → HARD STOP
 * ============================================================
 *  COMMANDS:
...
int   creepSpeed           = 30;    // BUG1 FIX C: slow creep during dark confirm

// Timing constants
#define JUNCTION_PAUSE_MS        1000  // pause at junction before turning
#define ROOM_WAIT_MS             7000  // wait at room end
#define UTURN_MIN_MS              800  // min spin ms before looking for line
#define UTURN_TIMEOUT_MS         4000  // switch uturn direction after this
...
// BUG 1 FIXES
#define BRANCH_MIN_TRAVEL_MS     2000  // FIX A: arm end-detect only after this
#define LINE_END_CONFIRM_MS      1500  // FIX B: was 600 → 1500ms

// Normal outbound junction detection
#define JUNCTION_FRAMES_NEEDED      3
#define JUNCTION_COOLDOWN_MS      900

// BUG 2 FIXES — stricter junction detection on return leg
#define RETURN_JUNCTION_FRAMES      6  // FIX A: needs 6 frames (vs 3)
#define RETURN_JUNCTION_COOLDOWN  2000 // FIX B: 2000ms cooldown (vs 900ms)
#define RETURN_MIN_TRAVEL_MS     1500  // FIX C: wait before counting starts
...
unsigned long branchStartTime    = 0;   // BUG1 FIX A
unsigned long returnStartTime    = 0;   // BUG2 FIX C
bool          lineEndActive      = false;
bool          endDetectArmed     = false; // BUG1 FIX A
...
void doPD();
bool checkJunction();
bool checkJunctionStrict();   // BUG2 FIX A — stricter version for return
...
//  BUG2 FIX A: needs 6 frames + 2000ms cooldown
// ─────────────────────────────────────────────────────────────
bool checkJunctionStrict() {
  IRSensor ir  = readIR();
  int      sum = irSum(ir);

  if (sum >= 4) {   // require 4+ sensors (real T-junction wider than line)
...
void doLineFollow() {
  ...
  if (state == TURNING_TO_ROOM) {
    if (spinLeft(BRANCH_TURN_MIN_MS)) {
      ...
      junctionFrames   = 0;
      lastJunctionTime = millis();
      branchStartTime  = millis();   // BUG1 FIX A: start timer
      state            = ON_BRANCH;
  ...
  if (state == ON_BRANCH) {
    ...
    // BUG1 FIX A: arm end detection only after minimum travel time
    if (!endDetectArmed) {
      unsigned long travelled = millis() - branchStartTime;
      if (travelled < BRANCH_MIN_TRAVEL_MS) {
        // Not yet armed — just follow line, ignore all-dark
        doPD();
    ...
    // BUG1 FIX B+C: longer confirm window + slow creep during confirm
    if (allDark(ir)) {
      if (!lineEndActive) {
        lineEndActive = true;
        lineEndStart  = millis();
      ...
      // BUG1 FIX C: creep forward very slowly — grout line will reappear
      ...
      // Re-check — if sensors see line again it was a floor gap
      IRSensor recheck = readIR();
      if (!allDark(recheck)) {
        lineEndActive = false;
        Serial.println("  false end (floor gap) — line back, continuing");
        return;
      }
  ...
  if (state == REJOINING_MAIN) {
    if (spinRight(REJOIN_SPIN_MS)) {
      ...
      returnStartTime       = millis();   // BUG2 FIX C
      state                 = RETURNING;
  ...
  if (state == RETURNING) {
    ...
    // BUG2 FIX C: don't count junctions until minimum travel done
    ...
    // BUG2 FIX A+B: use strict junction check on return
...
void setup() {
  ...
  Serial.println("==============================================");
  Serial.println("  LINE FOLLOW v23 — Hospital Rover           ");
  Serial.println("==============================================");
  Serial.println("  A=Room1  B=Room2  C=end  then 'g'          ");
  Serial.println("  BUG1: end-detect armed after 2s on branch  ");
  Serial.println("  BUG1: 1500ms confirm + creep (not stop)    ");
  Serial.println("  BUG2: strict 6-frame junction on return    ");
  Serial.println("  BUG2: 1500ms travel before counting starts ");
  Serial.println("==============================================");
  printSettings();
}
