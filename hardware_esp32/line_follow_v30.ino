/*
 * ============================================================
 *  LINE FOLLOW v30 — v29 State Machine + Supabase Order System
 * ============================================================
 *
 *  WHAT'S NEW vs v29:
 *   - WiFi + Supabase polling (from v14) integrated
 *   - Orders table polled every 5s when idle
 *   - room_code A → targetJunction=1 (left at J1)
 *   - room_code B → targetJunction=2 (left at J2)
 *   - room_code C → targetJunction=0 (straight to end)
 *   - On order pickup: status → "in_transit", bot starts
 *   - On missionComplete(): status → "delivered", idle
 *   - Manual 'g' mode still works (skips Supabase)
 *
...
void setup() {
  ...
  Serial.println("==============================================");
  Serial.println("  LINE FOLLOW v30 — Hospital Rover           ");
  Serial.println("  v29 state machine + Supabase order system  ");
  Serial.println("==============================================");
...
void loop() {
  ...
  Serial.printf("[poll] Order #%ld -> room %s (%s)\n",
    o.id, o.roomCode.c_str(), o.roomLabel.c_str());

  // Map room_code to junction number
  int junction = -1;
  if      (o.roomCode == "A") junction = 1;
  else if (o.roomCode == "B") junction = 2;
  else if (o.roomCode == "C") junction = 0;
  ...
  // Mark in_transit first — if this fails don't start
  if (!updateOrderStatus(o.id, "in_transit")) {
    Serial.println("[poll] Could not mark in_transit — aborting");
    return;
  }
