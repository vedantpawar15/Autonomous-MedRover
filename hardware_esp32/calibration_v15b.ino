...
void doLineFollow() {
  ...
  if (sum >= 3) {
    ...
    // While in junction zone — SLOW DOWN to give time to confirm
    int jSpeed = baseSpeed / 2;  // half speed through junction
    setMotorLeft(jSpeed);
    setMotorRight(constrain(jSpeed + rightOffset, 0, 255));
    return;
  }
